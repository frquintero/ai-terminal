...
6. Latency for literal shell queries (sub‑500 ms target): introduce a planner bypass for queries that are unambiguously shell commands (single-line, non-interactive, safe flags). The Orchestrator constructs a single-step plan object (with `tool_name` preset to `run_command` or `run_interactive` as needed) and calls Agent B directly, avoiding an Agent A hop. Roles remain intact (B still generates `output_format`), while we eliminate an unnecessary planning call. Fallback to full A→B when the input is ambiguous.
7. Acceptance criteria: identical or better latency than v2.0 for `ls`, `cat`, `rg`, etc.; correct `run_interactive` selection for interactive commands; zero regressions in safety guards.
