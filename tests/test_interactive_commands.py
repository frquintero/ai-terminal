...
from orchestrator.command_patterns import INTERACTIVE_PATTERNS_COMPILED
...
class TestInteractiveCommandDetection:
    ...
    def test_detect_vim(self):
        ...
        assert is_interactive_command("vim /etc/hosts") is True
        assert is_interactive_command("vim file.txt") is True
        assert is_interactive_command("VIM file.txt") is True  # Case insensitive
    
    def test_detect_vim_with_c_flag_is_not_interactive(self):
        """vim -c is non-interactive batch mode"""
        assert is_interactive_command("vim -c ':wq'") is False
        assert is_interactive_command("vim -c 'set number | wq'") is False
    
    def test_detect_nano(self):
        """Verify nano is detected as interactive"""
        assert is_interactive_command("nano file.py") is True
        assert is_interactive_command("nano ~/.bashrc") is True
    ...
    def test_detect_man(self):
        ...
        assert is_interactive_command("man 5 hosts") is True
    ...
    def test_detect_psql(self):
        ...
        assert is_interactive_command("psql mydb") is True
    ...
    def test_regular_commands_not_detected_as_interactive(self):
        ...
        assert is_interactive_command("cat /etc/hosts") is False
        assert is_interactive_command("grep text file.txt") is False
...
class TestInteractiveCommandRouting:
    ...
    def test_vim_routes_to_shell(self):
        ...
        result = classify_query("vim /etc/hosts")
...
class TestInteractiveCommandInfo:
    ...
    def test_rule_engine_reports_interactive_patterns(self):
        """Interactive pattern registry should remain sizable."""
        assert len(INTERACTIVE_PATTERNS_COMPILED) >= 20
